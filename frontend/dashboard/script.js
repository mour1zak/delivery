// Dashboard Controller - VERSÃO COMPLETA
class DashboardController {
    ...
    renderizarClientesCarrossel(ranking) {
        ...
        const cardsPorPagina = Math.min(4, totalCards);
                ...
                <div class="cliente-card-flip-inner">
                    <div class="cliente-card-front">
                        ...
                        <div class="cliente-avatar">
                            <img src="${avatarImg}" alt="${item.nome}" class="cliente-avatar-img">
                        </div>
    ...
    // ============================================
    // GPS COM CÁLCULO DE FRETE E DISTÂNCIA
    // ============================================
    ...
    inicializarGPS() {
        ...
        const bgImage = 'https://images.pexels.com/photos/26173280/pexels-photo-26173280.jpeg';
        
        pedidosEmEntrega.forEach((pedido) => {
            const cliente = this.clientes.find(c => c.id === pedido.clienteId);
            if (!cliente) return;
            
            const cepRestaurante = '08485-300';
            const cepCliente = cliente.cep || '00000-000';
            const distancia = Helpers.calcularDistancia(cepRestaurante, cepCliente);
            const tempoEstimado = Helpers.calcularTempoEstimado(distancia);
            const frete = Helpers.calcularFrete(distancia);
            
            gpsContent.innerHTML += `
                <div class="gps-card" style="background-image: url('${bgImage}'); background-size: cover; background-position: center; position: relative;">
                    <div class="gps-card-overlay"></div>
                    <div class="gps-card-content" style="position: relative; z-index: 2;">
                        <div class="gps-card-header">
                            <h4>🚀 Pedido ${pedido.id} - ${cliente.nome.toUpperCase()}</h4>
                        ...
                        <div class="gps-info-grid">
                            <div class="gps-info-item">
                                ...
                                <div class="value">${distancia.toFixed(1)} km</div>
                            </div>
                            <div class="gps-info-item">
                                <div class="label">⏱️ Tempo estimado</div>
                                <div class="value">${tempoEstimado} min</div>
                            </div>
                            <div class="gps-info-item" style="background: rgba(251, 250, 218, 0.08); border: 1px solid rgba(251, 250, 218, 0.1);">
                                <div class="label">💰 Frete</div>
                                <div class="value" style="color: #FBFADA; font-size: 1.1rem;">${Helpers.formatarMoeda(frete)}</div>
                            </div>
                            <div class="gps-info-item">
                        ...
                        <div class="gps-endereco">
                            <span class="endereco-icon">📍</span>
                            <span class="endereco-text">${cliente.endereco || 'Endereço não informado'}${cliente.bairro ? `, ${cliente.bairro}` : ''}${cliente.cep ? ` - ${cliente.cep}` : ''}</span>
                        </div>
                    </div>
                </div>
            `;
